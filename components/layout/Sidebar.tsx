    ...
    return (
        <aside className={`bg-white border-r border-slate-200 flex flex-col h-full ${className}`}>
            <div className="h-16 flex items-center px-6 gap-3 border-b border-slate-100 flex-shrink-0 mb-4">
                <div className="w-10 h-10 bg-orange-500 rounded-xl flex items-center justify-center font-black text-white text-xl shadow-lg">B</div>
                <div>
                    <h1 className="font-black text-slate-800 text-base leading-tight tracking-tight uppercase">Belare</h1>
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Inteligente</p>
                </div>
            </div>
            ...
            <nav className="flex-1 overflow-y-auto p-4 scrollbar-hide">
                <div className="mb-6">
                    ...
                    <ul className="space-y-1">
                        {filter(principalItems).map(renderItem)}
                        {filter(operacionalItems).map(renderItem)}
                    </ul>
                </div>
                ...
                <div className="mb-6">
                    ...
                    <ul className="space-y-1">
                        {filter(gestaoItems).map(renderItem)}
                    </ul>
                </div>
            </nav>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50/50">
                <div className="flex items-center gap-3 w-full p-3 rounded-2xl bg-white border border-slate-100 shadow-sm">
                    <img 
                        src={user?.avatar_url || `https://ui-avatars.com/api/?name=${user?.nome || 'U'}&background=random`} 
                        className="w-10 h-10 rounded-full border border-orange-100 object-cover"
                    />
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-black text-slate-700 truncate">{user?.nome || 'Usuário'}</p>
