...
// FIX: Removed startOfDay, setHours, and setMinutes as they were reported as missing exports from 'date-fns'.
import { format, addMinutes, isSameDay, addDays } from "date-fns";
// FIX: Corrected locale import path to 'date-fns/locale/pt-BR' to resolve "no exported member 'ptBR'" error.
import { ptBR } from "date-fns/locale/pt-BR";
import { 
  ChevronLeft, ChevronRight, 
  Clock, User, Check, X, Plus, Loader2
} from 'lucide-react';
import { supabase } from "../../services/supabaseClient";
// FIX: Imported FinancialTransaction to define the prop type.
...
// FIX: Added onAddTransaction prop to match caller (App.tsx) expectations.
export default function AtendimentosView({ onAddTransaction }: { onAddTransaction?: (t: FinancialTransaction) => void }) {
  // --- Estados ---
  ...
  // Memos de Data
  // FIX: Replaced startOfDay with native Date logic to avoid missing member error.
  ...
  // FIX: Replaced setMinutes/setHours with native Date logic to avoid missing member errors.
  ...
  function handleGridDoubleClick(e: React.MouseEvent, profId: string) {
    ...
    // FIX: Replaced setMinutes/setHours with native Date logic.
  ...
  function openNewAt(start = new Date(), profId = professionals[0]?.id) {
    ...
    setEditing({
      ...
      ends_at: addMinutes(s, 60).toISOString(),
      status: "scheduled",
      notes: ""
    });
    setModalOpen(true);
              ...
              >
                ...
                >
                   ...
                   {appointmentsByProf[prof.id]?.map(appt => {
                     ...
                     const colorClass = PROF_COLORS[idx % PROF_COLORS.length];
