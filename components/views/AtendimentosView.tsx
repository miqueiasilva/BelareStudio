...
const AtendimentosView: React.FC<AtendimentosViewProps> = ({ onAddTransaction }) => {
    ...
    const fetchResources = async () => {
        ...
        try {
            ...
            console.error("Erro ao carregar recursos da agenda:", e); 
            if (isMounted.current) {
                setToast({ message: "Erro ao carregar profissionais: " + (e.message || "Falha no banco"), type: 'error' });
            }
        }
    };
    ...
    const fetchAppointments = async () => {
        ...
        const watchdog = setTimeout(() => {
            if (isMounted.current && isLoadingData) {
                ...
                setError("A sincronização demorou muito.");
            }
        }, 8000);
        ...
        try {
            ...
            if (isMounted.current && e.name !== 'AbortError') {
                ...
                setError(e.message || "Erro de conexão com a agenda.");
            }
        } finally { 
                ...
                <AppointmentModal 
                    ...
                    onSave={async (app) => {
                        try {
                            ...
                            setModalState(null);
                            fetchAppointments();
                        } catch (e: any) { setToast({ message: 'Erro ao salvar agendamento.', type: 'error' }); }
                    }} 
                />
            )}
                ...
                <BlockTimeModal 
                    ...
                    onSave={async (block) => {
                        try {
                            ...
                            setModalState(null);
                            fetchAppointments();
                        } catch (e: any) { setToast({ message: 'Erro ao bloquear.', type: 'error' }); }
                    }} 
                />
            )}
