    ...
    const fetchResources = async () => {
        ...
        try {
            ...
            if (isMounted.current) {
                const msg = e?.message || "Falha ao carregar catálogo de profissionais.";
                setToast({ message: `Erro ao carregar recursos da agenda: ${msg}`, type: 'error' });
            }
        }
    };
    ...
    const fetchAppointments = async () => {
        ...
        const watchdog = setTimeout(() => {
            if (isMounted.current && isLoadingData) {
                ...
                setError("A sincronização dos dados expirou.");
            }
        }, 8000);
        ...
        try {
            ...
            if (data && isMounted.current) {
                const mapped = data.map(row => {
                    ...
                    return {
                        ...
                        professional: resources.find(p => p.id === Number(row.resource_id)) || { id: 0, name: row.professional_name || 'Profissional', avatarUrl: '' },
                        service: { id: 0, name: row.service_name || 'Serviço', price: Number(row.value), duration: dur, color: row.status === 'bloqueado' ? '#64748b' : '#3b82f6' }
            ...
            if (isMounted.current && e.name !== 'AbortError') {
                console.error("Falha no fetch de agendamentos:", e);
                setError(e?.message || "Erro inesperado ao carregar dados da agenda.");
            }
        } finally { 
            ...
            <div className="flex-1 overflow-auto bg-slate-50 relative custom-scrollbar">
                {error ? (
                    <div className="flex flex-col items-center justify-center h-full text-slate-400 p-8 text-center">
                        ...
                        <h3 className="font-bold text-slate-700">Erro na Agenda</h3>
                        <p className="max-w-xs text-sm mt-1 mb-6">{error}</p>
                        <button onClick={fetchAppointments} className="bg-slate-800 text-white px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2"><RefreshCw size={14}/> Sincronizar Agora</button>
                    </div>
                ) : (
                                ...
                                <div 
                                    ...
                                    onClick={(e) => {
                                        if (e.target === e.currentTarget) {
                                            const prof = col.type === 'professional' ? (col.data as LegacyProfessional) : (resources[0] || { id: 0, name: 'Recepção', avatarUrl: '' });
            ...
            {activeAppointmentDetail && (
                <AppointmentDetailPopover 
                    ...
                    onDelete={async (id) => { 
                        ...
                        } catch (e: any) { setToast({ message: e.message || "Erro ao excluir agendamento.", type: 'error' }); }
                    }} 
                    onUpdateStatus={async (id, status) => { 
                        try {
                            ...
                            setActiveAppointmentDetail(null); 
                        } catch (e: any) { setToast({ message: e.message || "Erro ao atualizar status.", type: 'error' }); }
                    }} 
                />
            )}
                ...
                <AppointmentModal 
                    ...
                    onSave={async (app) => {
                        try {
                            ...
                            setModalState(null);
                            fetchAppointments();
                        } catch (e: any) { setToast({ message: e.message || 'Erro ao salvar agendamento.', type: 'error' }); }
                    }} 
                />
            )}
                ...
                <BlockTimeModal 
                    ...
                    onSave={async (block) => {
                        try {
                            ...
                            setModalState(null);
                            fetchAppointments();
                        } catch (e: any) { setToast({ message: e.message || 'Erro ao bloquear horário.', type: 'error' }); }
                    }} 
                />
            )}
