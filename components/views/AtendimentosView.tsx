...
const AtendimentosView: React.FC<AtendimentosViewProps> = ({ onAddTransaction }) => {
    ...
    const fetchResources = async () => {
        try {
            // MIGRADO: Fonte de dados agora é 'team_members'
    ...
    return (
        <div className="flex h-full bg-white relative flex-col font-sans text-left">
            ...
            <header className="flex-shrink-0 bg-white border-b border-slate-200 px-6 py-4 z-30 shadow-sm">
                <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-2 w-full lg:w-auto justify-end">
                        <button onClick={() => setIsConfigModalOpen(true)} className="p-2 rounded-lg border border-slate-300 bg-white text-slate-500 hover:bg-slate-50 transition-all" title="Configurações de Exibição"><SlidersHorizontal size={20} /></button>
            ...
            <div className="flex-1 overflow-auto bg-slate-50 relative custom-scrollbar">
                <div className="min-w-fit">
                            ...
                            <div key={col.id} className="flex flex-col items-center justify-center p-2 border-r border-slate-100 h-24 bg-slate-50/10 relative group transition-colors hover:bg-slate-50">
                                ...
                                {periodType === 'Dia' && col.type === 'professional' && (
                                    <div className="absolute bottom-1 left-1/2 -translate-x-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={(e) => { e.stopPropagation(); moverEsq(idx); }} disabled={idx === 0} className="p-1 bg-white border border-slate-200 rounded shadow-sm text-slate-400 hover:text-orange-500 disabled:opacity-30"><ChevronLeft size={14} /></button>
                                        <button onClick={(e) => { e.stopPropagation(); moverDir(idx); }} disabled={idx === orderedProfessionals.length - 1} className="p-1 bg-white border border-slate-200 rounded shadow-sm text-slate-400 hover:text-orange-500 disabled:opacity-30"><ChevronRight size={14} /></button>
                                    </div>
                                )}
                            </div>
                        ))}
                    ...
                    <div className="grid relative" style={{ gridTemplateColumns: `60px repeat(${columns.length}, minmax(${isAutoWidth ? '180px' : colWidth + 'px'}, 1fr))` }}>
                        ...
                        {columns.map((col, idx) => (
                            <div key={col.id} className={`relative border-r border-slate-200 min-h-[1000px] cursor-crosshair ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/[0.03]'}`} onClick={(e) => { if (e.target === e.currentTarget) { const prof = col.type === 'professional' ? (col.data as LegacyProfessional) : resources[0]; const date = col.type === 'date' ? (col.data as Date) : currentDate; handleGridClick(e, prof, date); } }}>
                                ...
                                {filteredAppointments.filter(app => (periodType === 'Semana' ? isSameDay(app.start, col.data as Date) : (app.professional.id === col.id || app.professional.name === col.title))).map(app => (
                                    <div key={app.id} ref={(el) => { if (el) appointmentRefs.current.set(app.id, el); }} onClick={(e) => { e.stopPropagation(); setActiveAppointmentDetail(app); }} title={`${format(app.start, 'HH:mm')} - ${app.client?.nome || 'Bloqueado'} (${app.service.name})`} className={getCardStyle(app, viewMode)} style={{ ...getAppointmentPosition(app.start, app.end, timeSlot), borderLeftColor: app.service.color }}>
                                        <div className="absolute top-1.5 right-1.5 opacity-40 group-hover/card:opacity-100 transition-opacity">
                                            {app.origem === 'link' ? <Globe size={10} className="text-orange-500" title="Agendado pelo Site" /> : <User size={10} className="text-slate-400" title="Agendado Manualmente" />}
                                        </div>
                                        ...
                                        <p className="text-[10px] font-medium text-slate-600 truncate leading-tight mt-0.5">{app.service.name}</p>
                                    </div>
                                ))}
                            </div>
                        ))}
            ...
            {/* Restante da UI de modais omitida para brevidade, mantida funcional no app final */}
        </div>
    );
};
