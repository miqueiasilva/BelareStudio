...
const RemuneracoesView: React.FC = () => {
  ...
  const payroll = useMemo(() => {
    return teamMembers.map(member => {
      ...
      // ✅ REGRA: Comissão sobre o Líquido (net_value)
      const totalBase = myTrans.reduce((acc, t) => {
          const baseValue = Number(t.net_value || t.amount || 0);
  ...
  return (
    <div className="p-6 h-full overflow-y-auto bg-slate-50/50 font-sans text-left">
      <header className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div><h1 className="text-2xl font-black text-slate-800 flex items-center gap-2"><Wallet className="text-orange-500" /> Remunerações</h1><p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Cálculo de repasse baseado em valores líquidos (net_value)</p></div>
            ...
            <div key={item.member.id} className="bg-white border border-slate-100 rounded-[32px] overflow-hidden shadow-sm transition-all hover:shadow-md">
                <div className="flex items-center justify-between p-6 cursor-pointer hover:bg-slate-50/50" onClick={() => setExpandedId(expandedId === item.member.id ? null : item.member.id)}>
                    ...
                    <div className="flex items-center gap-10">
                        <div className="text-right"><p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mb-0.5">Base de Cálculo (Net)</p><p className="text-sm font-bold text-slate-400">{formatBRL(item.totalBase)}</p></div>
                ...
                {expandedId === item.member.id && (
                     <div className="bg-slate-50/50 border-t border-slate-100 p-8 animate-in slide-in-from-top-4 duration-300">
                        <div className="flex items-center justify-between mb-6"><h4 className="text-xs font-black text-slate-600 uppercase tracking-[0.2em]">Detalhamento das Vendas Liquidadas</h4><div className="bg-blue-50 text-blue-700 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border border-blue-100"><Info size={14} /> Base calculada sobre o valor líquido (pós-taxas)</div></div>
                        {item.transactions.length > 0 ? (
                            <div className="bg-white rounded-[24px] border border-slate-200 overflow-hidden shadow-sm mb-8">
                                <table className="w-full text-sm text-left"><thead className="text-[10px] text-slate-400 uppercase font-black bg-slate-50/50 tracking-widest border-b border-slate-100"><tr><th className="px-8 py-5">Item / Serviço</th><th className="px-8 py-5">Data</th><th className="px-8 py-5 text-right">Líquido (Net)</th><th className="px-8 py-5 text-right text-orange-600">Comissão ({item.rate}%)</th></tr></thead>
                                    <tbody className="divide-y divide-slate-100">
                                        {item.transactions.map((t: any) => {
                                            const base = Number(t.net_value || t.amount || 0);
