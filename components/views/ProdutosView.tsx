        ...
        <div className="h-full flex flex-col bg-slate-50 font-sans text-left overflow-hidden">
            ...
            <header className="bg-white border-b border-slate-200 px-6 py-4 flex flex-col lg:flex-row justify-between items-center gap-4 shadow-sm z-30 flex-shrink-0">
                ...
                <div className="flex items-center gap-3 w-full lg:w-auto">
                    <button onClick={() => setIsMovementModalOpen(true)} className="flex-1 lg:flex-none px-5 py-3 bg-white border border-slate-200 text-slate-600 font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-slate-50 transition-all flex items-center justify-center gap-2 shadow-sm">
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                                <tr>
                                    ...
                                    <th className="px-8 py-5">Motivo / Observação</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                                {movements.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="px-8 py-20 text-center text-slate-400 font-bold uppercase tracking-widest">Nenhuma movimentação registrada</td>
                                    </tr>
                                ) : (
                                    movements.map(m => (
                                        <tr key={m.id} className="hover:bg-slate-50 transition-all">
                                            ...
                                            <td className={`px-8 py-5 text-center font-black ${m.type === 'in' ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                {m.type === 'in' ? '+' : '-'}{m.quantity}
                                            </td>
                                            <td className="px-8 py-5 text-xs text-slate-500 font-medium italic">
                                                {m.reason}
                                            </td>
                                        </tr>
                                    ))
