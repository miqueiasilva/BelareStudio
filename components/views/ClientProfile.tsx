        ...
        try {
            ...
            if (anamnesis.signature_url) {
                try {
                    ...
                    y += 4;
                    doc.text(`Assinado em ${format(new Date(anamnesis.signed_at), 'dd/MM/yyyy HH:mm')}`, pageWidth / 2, y, { align: 'center' });
                } catch (imgErr) {
                    ...
                    y += 5;
                    doc.setFontSize(7);
                    doc.text(`Timestamp: ${format(new Date(anamnesis.signed_at), 'dd/MM/yyyy HH:mm')}`, pageWidth / 2, y, { align: 'center' });
                }
            }
            ...
            if (navigator.share && /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
                ...
                await navigator.share({ files: [file], title: 'Contrato e Anamnese - Jacilene Félix', text: 'Documento assinado eletronicamente.' });
            } else {
                doc.save(fileName);
            }
                    ...
                    <div className="flex gap-2">
                        {!isEditing ? (
                            ...
                            <div className="flex gap-2">
                                ...
                                <button type="button" onClick={handleSave} disabled={isSaving} className="px-6 py-2.5 bg-orange-600 text-white font-bold rounded-xl flex items-center gap-2 shadow-lg shadow-orange-100 transition-all active:scale-95 disabled:opacity-70">{isSaving ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}{isSaving ? 'Gravando...' : (isNew ? 'Criar Cliente' : 'Confirmar Dados')}</button>
                            </div>
                        )}
                    </div>
                </div>
                <div className="flex items-center gap-5">
                    <div className="relative group">
                        <div className="relative">
                            <div onClick={() => isEditing && avatarInputRef.current?.click()} className={`w-20 h-20 rounded-[24px] flex items-center justify-center text-2xl font-black border-4 border-white shadow-xl overflow-hidden transition-all relative ${isEditing ? 'cursor-pointer hover:ring-4 hover:ring-orange-100 group' : ''} ${formData.photo_url ? 'bg-white' : 'bg-orange-100 text-orange-600'} ${isUploading ? 'opacity-50' : 'opacity-100'}`}>
                                {formData.photo_url ? <img src={formData.photo_url} className="w-full h-full object-cover" alt="Avatar" /> : formData.nome?.charAt(0) || '?'}
                                {isEditing && <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">{isUploading ? <Loader2 className="text-white animate-spin" size={20} /> : <Camera className="text-white" size={20} />}</div>}
                            </div>
                            {isEditing && formData.photo_url && <button onClick={(e) => { e.stopPropagation(); handleRemoveAvatar(); }} className="absolute -bottom-1 -right-1 p-1.5 bg-white text-rose-500 rounded-lg shadow-lg border border-slate-100 hover:bg-rose-50 transition-all active:scale-90 z-20" title="Remover foto"><Trash2 size={14} /></button>}
                        </div>
                    </div>
                    <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                            ...
                            {(anamnesis.has_allergy || anamnesis.is_pregnant || anamnesis.uses_roacutan) && <span className="text-[10px] font-black uppercase text-rose-500 bg-rose-50 px-2 py-0.5 rounded-lg border border-rose-100 flex items-center gap-1"><AlertCircle size={10}/> Atenção Médica</span>}
                        </div>
                    </div>
                </div>
            </header>
                        ...
                        <div className="space-y-6 animate-in fade-in duration-500">
                             ...
                             <Card title="Perfil do Cliente" icon={<User size={18} />}>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <select name="sexo" value={formData.sexo} onChange={(e) => setFormData({ ...formData, sexo: e.target.value })} disabled={!isEditing} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-orange-50 disabled:opacity-60">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <select name="origem" value={formData.origem} onChange={(e) => setFormData({ ...formData, origem: e.target.value })} disabled={!isEditing} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-orange-50 disabled:opacity-60">
                             ...
                             <Card title="Observações Gerais" icon={<FileText size={18} />}>
                                <div className="space-y-1.5">
                                    ...
                                    <textarea name="observacoes" value={formData.observacoes} onChange={handleInputChange} disabled={!isEditing} className="w-full bg-white border border-slate-200 rounded-2xl p-4 min-h-[120px] outline-none focus:ring-4 focus:ring-orange-50 focus:border-orange-400 transition-all font-medium text-slate-600 resize-none shadow-sm disabled:opacity-60" placeholder="Ex: Prefere tons pastéis, tem sensibilidade nos cílios..." />
                                </div>
                             </Card>
                        </div>
                    )}
                                ...
                                <div className="space-y-8">
                                    <div className="p-5 bg-orange-50 border-2 border-orange-100 rounded-3xl mb-4 flex items-center justify-between group hover:border-orange-300 transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className="p-3 bg-white rounded-2xl text-orange-500 shadow-sm border border-orange-100"><ImageIcon size={20} /></div>
                                            <div>
                                        ...
                                        <ToggleSwitch on={anamnesis.photo_authorized} onClick={() => setAnamnesis({...anamnesis, photo_authorized: !anamnesis.photo_authorized})} />
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Valor Acordado (R$)</label>
                                            <div className="relative group">
                                                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-500"><DollarSign size={18} /></div>
                                                <input type="text" value={anamnesis.service_value} onChange={(e) => setAnamnesis({...anamnesis, service_value: e.target.value})} placeholder="Ex: 350,00" className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-orange-50 focus:border-orange-400 transition-all font-black text-slate-700 shadow-sm" />
                                            </div>
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Forma de Pagamento</label>
                                            <div className="relative group">
                                                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-500"><CreditCard size={18} /></div>
                                                <select value={anamnesis.payment_method} onChange={(e) => setAnamnesis({...anamnesis, payment_method: e.target.value})} className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none appearance-none focus:ring-4 focus:ring-orange-50 focus:border-orange-400 transition-all font-bold text-slate-700 shadow-sm">
                                                    <option value="">Selecione...</option>
                                    ...
                                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 space-y-6">
                                        <div className="flex items-center gap-2 mb-2"><ClipboardCheck className="text-orange-500" size={18} /><h4 className="text-xs font-black text-slate-700 uppercase tracking-widest">Avaliação Técnica (Profissional)</h4></div>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                            <div className="space-y-1.5">
                                                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Fototipo</label>
                                                <select value={anamnesis.phototype} onChange={(e) => setAnamnesis({...anamnesis, phototype: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-orange-50">
                                                    <option value="">Selecione</option>
                                            ...
                                            <div className="space-y-1.5">
                                                ...
                                                <select value={anamnesis.technique_applied} onChange={(e) => setAnamnesis({...anamnesis, technique_applied: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-orange-50">
                                                    <option value="">Selecione</option>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                                        {[
                                            { key: 'is_pregnant', label: 'Gestante ou Lactante?' },
                                            { key: 'has_allergy', label: 'Possui Alguma Alergia?', hasDetails: true, detailKey: 'allergy_details', placeholder: 'Cite substâncias...' },
                                            { key: 'uses_acids', label: 'Usa Ácidos no Rosto?', hasDetails: true, detailKey: 'acids_details', placeholder: 'Quais ácidos...' },
                                            { key: 'uses_roacutan', label: 'Usa ou usou Roacutan?', hasDetails: true, detailKey: 'roacutan_time', placeholder: 'Há quanto tempo...' },
                                            { key: 'has_diabetes', label: 'Diabetes?' },
                                            { key: 'has_keloid', label: 'Problema de Quelóide?' },
                                            { key: 'has_herpes', label: 'Histórico de Herpes Labial?' },
                                            { key: 'uses_anticoagulants', label: 'Usa Anticoagulantes?' },
                                            { key: 'recent_botox', label: 'Fez Botox recentemente (6 meses)?' },
                                            { key: 'has_autoimmune', label: 'Doenças Autoimunes?' },
                                            { key: 'cancer_treatment', label: 'Em Tratamento Oncológico (Câncer)?' },
                                            { key: 'has_dermatitis', label: 'Possui Dermatite ou Lesão no local?' },
                                            { key: 'aspirin_use', label: 'Tomou Aspirina nos últimos 5 dias?' }
                                        ].map(q => (
                                            <div key={q.key} className="space-y-3 pb-4 border-b border-slate-50 last:border-0 md:border-b-0">
                                                <div className="flex items-center justify-between gap-4"><p className="font-bold text-slate-700 text-sm">{q.label}</p><ToggleSwitch on={anamnesis[q.key]} onClick={() => setAnamnesis({...anamnesis, [q.key]: !anamnesis[q.key]})} /></div>
                                                {q.hasDetails && anamnesis[q.key] && <div className="animate-in slide-in-from-top-2 duration-300"><textarea value={anamnesis[q.detailKey]} onChange={(e) => setAnamnesis({...anamnesis, [q.detailKey]: e.target.value})} placeholder={q.placeholder} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-xs font-medium text-slate-600 outline-none focus:ring-2 focus:ring-orange-100 focus:border-orange-400 shadow-inner" rows={1} /></div>}
                                            </div>
                                        ))}
                                    </div>
                                    <div className="pt-4 space-y-4">
                                        <div className="flex flex-col sm:flex-row items-end gap-3 p-4 bg-orange-50 rounded-2xl border border-orange-100">
                                            <div className="flex-1 w-full space-y-1.5"><label className="text-[10px] font-black text-orange-600 uppercase tracking-widest ml-1">Carregar Modelo de Ficha ou Contrato</label><div className="relative"><select value={selectedTemplateId} onChange={(e) => setSelectedTemplateId(e.target.value)} className="w-full bg-white border border-orange-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 outline-none appearance-none focus:ring-2 focus:ring-orange-200"><option value="">Selecione um roteiro ou contrato...</option>{templates.map(t => (<option key={t.id} value={t.id}>{t.name}</option>))}</select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-orange-400 pointer-events-none" size={16} /></div></div>
                                            <div className="flex w-full sm:w-auto gap-2"><button onClick={handleLoadTemplate} disabled={!selectedTemplateId} className="flex-1 sm:flex-none px-6 py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-black rounded-xl text-xs flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50 shadow-sm"><FilePlus size={16} /> Inserir Modelo</button><button onClick={handleClearAnamnesisText} className="px-4 py-2.5 bg-white border border-slate-200 text-slate-400 hover:text-rose-500 hover:border-rose-100 rounded-xl text-xs flex items-center justify-center transition-all shadow-sm active:scale-95" title="Limpar tudo"><Eraser size={16} /></button></div>
                                        </div>
                                        <div className="flex justify-between items-center px-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Observações Clínicas / Termos Adicionais</label><button onClick={handleGeneratePDF} disabled={isLoading} className="text-[10px] font-black text-orange-600 uppercase tracking-widest flex items-center gap-2 px-3 py-1 bg-white border border-orange-200 rounded-lg hover:bg-orange-50 transition-colors disabled:opacity-50">{isLoading ? <Loader2 size={12} className="animate-spin" /> : <Share2 size={12} />}Compartilhar Contrato (PDF)</button></div>
                                        <textarea value={anamnesis.clinical_notes} onChange={(e) => setAnamnesis({...anamnesis, clinical_notes: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm font-medium text-slate-600 outline-none focus:ring-2 focus:ring-orange-100" rows={12} placeholder="Anotações internas..." />
                                    </div>
                                    <div className="pt-6 border-t border-slate-100">
                                        <div className="flex justify-between items-end mb-4"><h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2"><PenTool size={14}/> Assinatura do Termo de Ciência</h4>{anamnesis.signed_at && <span className="text-[9px] bg-emerald-50 text-emerald-600 px-2 py-1 rounded-lg font-black uppercase border border-emerald-100">Assinado em {format(new Date(anamnesis.signed_at), 'dd/MM/yy HH:mm')}</span>}</div>
                                        {anamnesis.signature_url ? (<div className="relative group bg-slate-50 rounded-3xl border-2 border-slate-100 p-4 h-56 flex items-center justify-center overflow-hidden shadow-inner"><img src={anamnesis.signature_url} className="max-h-full max-w-full object-contain mix-blend-multiply" alt="Assinatura" /><button onClick={() => setAnamnesis({...anamnesis, signature_url: null, signed_at: null})} className="absolute top-4 right-4 p-2 bg-white text-rose-500 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2 text-[10px] font-bold uppercase border border-rose-50"><Trash2 size={14} /> Refazer</button></div>) : (<SignaturePad onSave={handleSaveSignature} isSaving={isSaving} />)}
                                    </div>
                                    <button onClick={handleSaveAnamnesis} disabled={isSaving} className="w-full py-4 bg-slate-800 text-white font-black rounded-2xl hover:bg-slate-900 shadow-xl transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-70">{isSaving ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}Salvar Ficha de Anamnese</button>
                                </div>
                            </Card>
                        </div>
                    )}
