        ...
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 font-sans">
                <div className="bg-white p-8 rounded-3xl shadow-xl text-center max-w-sm w-full animate-in zoom-in-95 duration-500">
                    ...
                    <p className="text-slate-500 mb-8">Obrigado, <b>{clientForm.name.split(' ')[0]}</b>. Já reservamos seu horário.</p>
                    <div className="bg-slate-50 rounded-2xl p-4 mb-6 border border-slate-100 text-left">
                        ...
                        <p className="font-semibold text-slate-800">{format(selectedDate, "dd 'de' MMMM", { locale: pt })} às {selectedTime}</p>
                    </div>
                    <button className="w-full bg-green-500 text-white font-bold py-3 rounded-xl hover:bg-green-600 transition shadow-lg shadow-green-200 mb-3">Receber comprovante no WhatsApp</button>
                    <button onClick={() => window.location.reload()} className="text-slate-400 hover:text-slate-600 text-sm font-medium">Voltar ao início</button>
                </div>
            </div>
        );
    }
                ...
                <div className="max-w-md mx-auto">
                    {step === 'service' ? (
                        ...
                        <div className="flex items-center justify-between w-full">
                            <button onClick={handleBack} className="flex items-center gap-1 text-slate-600 hover:text-orange-600 transition-colors py-2 pr-3 -ml-2 rounded-lg group">
                                <div className="bg-slate-100 group-hover:bg-orange-100 rounded-full p-1.5 transition-colors"><ChevronLeft size={16} /></div>
                                <span className="font-semibold text-sm">Voltar</span>
                            </button>
                            <span className="font-bold text-slate-800 text-sm md:text-base">{step === 'professional' ? 'Profissional' : step === 'datetime' ? 'Horário' : 'Seus Dados'}</span>
                            <div className="w-16"></div>
                        </div>
                    )}
                    {step !== 'service' && (
                        <div className="h-1 bg-slate-100 w-full mt-4 rounded-full overflow-hidden">
                            <div className="h-full bg-orange-500 transition-all duration-500 ease-out" style={{ width: step === 'professional' ? '33%' : step === 'datetime' ? '66%' : '100%' }}></div>
                        </div>
                    )}
                </div>
            </div>
                ...
                {step === 'service' && (
                    <div className="space-y-6 animate-in slide-in-from-bottom-4 fade-in duration-500">
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Buscar procedimento..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-3.5 bg-white border-none shadow-sm rounded-2xl text-slate-700 focus:ring-2 focus:ring-orange-100 outline-none" />
                        </div>
                        {Object.entries(groupedServices).map(([cat, items]) => (
                            <div key={cat} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                                <button onClick={() => setExpandedCategories(prev => ({...prev, [cat]: !prev[cat]}))} className="w-full flex items-center justify-between p-4 bg-white border-b border-slate-50">
                                ...
                                {expandedCategories[cat] && (
                                    <div className="divide-y divide-slate-50">
                                        {(items as LegacyService[]).map(service => {
                                            ...
                                            return (
                                                <div key={service.id} onClick={() => setSelectedServiceIds(prev => isSelected ? prev.filter(id => id !== service.id) : [...prev, service.id])} className={`p-4 flex justify-between items-center cursor-pointer transition-colors ${isSelected ? 'bg-orange-50' : 'hover:bg-slate-50'}`}>
                                                    <div className="flex gap-3 items-start">
                                                        <div className={`mt-1 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${isSelected ? 'bg-orange-500 border-orange-500' : 'border-slate-300'}`}>{isSelected && <Check size={14} className="text-white" />}</div>
                                                        <div><p className={`font-semibold text-sm ${isSelected ? 'text-orange-900' : 'text-slate-700'}`}>{service.name}</p><p className="text-xs text-slate-400 mt-0.5">{formatDuration(service.duration)}</p></div>
                                                    </div>
                                                    <span className={`text-sm font-bold ${isSelected ? 'text-orange-700' : 'text-slate-600'}`}>R$ {service.price.toFixed(2)}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                        ...
                        <div className="bg-white p-5 rounded-2xl shadow-sm">
                            ...
                            <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide -mx-2 px-2">
                                {dates.map(date => {
                                    ...
                                    return (
                                        <button key={date.toISOString()} onClick={() => setSelectedDate(date)} className={`flex-shrink-0 w-16 h-20 rounded-2xl flex flex-col items-center justify-center border-2 transition-all ${selected ? 'bg-slate-800 text-white border-slate-800 shadow-lg scale-105' : 'bg-white text-slate-500 border-transparent hover:bg-slate-50'}`}>
                        ...
                        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-5">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus-within:ring-2 focus-within:ring-orange-200 transition-all">
                                        <User className="w-5 h-5 text-slate-400" /><input type="text" value={clientForm.name} onChange={(e) => setClientForm({...clientForm, name: e.target.value})} className="w-full bg-transparent outline-none text-slate-800" placeholder="Seu nome" />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">WhatsApp <span className="text-red-500">*</span></label>
                                    <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus-within:ring-2 focus-within:ring-orange-200 transition-all">
                                        <Phone className="w-5 h-5 text-slate-400" /><input type="tel" value={clientForm.phone} onChange={handlePhoneChange} maxLength={15} className="w-full bg-transparent outline-none text-slate-800" placeholder="(00) 00000-0000" />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">E-mail (Opcional)</label>
                                    <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus-within:ring-2 focus-within:ring-orange-200 transition-all">
                                        <Mail className="w-5 h-5 text-slate-400" /><input type="email" value={clientForm.email} onChange={(e) => setClientForm({...clientForm, email: e.target.value})} className="w-full bg-transparent outline-none text-slate-800" placeholder="seu@email.com" />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Observações</label>
                                    <textarea value={clientForm.notes} onChange={(e) => setClientForm({...clientForm, notes: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-orange-500 outline-none h-24 resize-none transition-all" placeholder="Alguma preferência ou alergia?" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="fixed bottom-0 left-0 w-full p-4 bg-white border-t border-slate-100 z-30 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
                    <div className="max-w-md mx-auto flex items-center justify-between gap-4">
                        <div className="flex flex-col">
                            <span className="text-[10px] text-slate-400 font-bold uppercase">{selectedServicesList.length} serviço(s)</span>
                            <div className="flex items-baseline gap-1"><span className="font-extrabold text-slate-800 text-xl">R$ {totalStats.price.toFixed(2)}</span><span className="text-xs font-medium text-slate-500">{formatDuration(totalStats.duration)}</span></div>
                        </div>
                        <button 
                            onClick={step === 'form' ? handleConfirmBooking : handleNext}
                            disabled={(step === 'datetime' && !selectedTime) || (step === 'form' && (!clientForm.name || !clientForm.phone)) || isSaving}
                            className="bg-slate-900 text-white font-bold py-3.5 px-8 rounded-xl disabled:opacity-50 hover:bg-black transition-all shadow-lg active:scale-95 flex items-center gap-2 min-w-[160px] justify-center"
                        >
                            {isSaving ? <Loader2 className="animate-spin w-5 h-5" /> : (step === 'form' ? 'Confirmar' : 'Continuar')}
                        </button>
                    </div>
                </div>
            )}
