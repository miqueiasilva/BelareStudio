...
import { DollarSign, Calendar, Users, TrendingUp, PlusCircle, UserPlus, ShoppingBag, Clock, Globe, Edit3, Loader2, BarChart3, AlertTriangle, RefreshCw } from 'lucide-react';
...
const DashboardView: React.FC<{onNavigate: (view: ViewState) => void}> = ({ onNavigate }) => {
    ...
    const fetchDashboardData = async () => {
        ...
        // Protocolo Antitrava: Watchdog de 10 segundos
        const watchdog = setTimeout(() => {
            if (isLoading && isMounted.current) {
                setIsLoading(false);
                setError("A conexão com o servidor demorou muito. Verifique sua rede.");
            }
        }, 10000);
        ...
        try {
            ...
            if (rpcError) {
                // Captura erro do Supabase de forma segura (evita [object Object])
                throw new Error(rpcError.message || "Erro ao consultar resumo de dados.");
            }
            ...
            if (isMounted.current) {
                ...
                setError(e.message || "Não foi possível carregar os indicadores estratégicos.");
            }
        } finally {
        ...
        return (
            <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-slate-50">
                <div className="bg-white p-10 rounded-[40px] shadow-xl border border-slate-100 max-w-md w-full animate-in zoom-in-95">
                    ...
                    <h3 className="font-extrabold text-slate-800 text-xl mb-2">Erro ao carregar Dashboard</h3>
