...
const DashboardView: React.FC<{onNavigate: (view: ViewState) => void}> = ({ onNavigate }) => {
    ...
    useEffect(() => {
        ...
        const fetchDashboardData = async () => {
            try {
                ...
                // Substituindo nomes de colunas inexistentes por Joins reais
                ...
                // Faturamento mensal consolidado via Relational Query
                const now = new Date();
    ...
    const kpis = useMemo(() => {
        // CORREÇÃO: Lê o preço do serviço relacionado (Join) em vez de coluna 'value'
    ...
    if (isLoading) {
        return (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 bg-white">
                ...
                <p className="text-xs font-black uppercase tracking-[0.3em] animate-pulse">Sincronizando Relatórios...</p>
            </div>
        );
    }
                    ...
                    <div className="flex flex-col gap-2 items-end">
                        ...
                        {filter === 'custom' && (
                            <div className="flex items-center gap-2 animate-in slide-in-from-top-2 duration-300">
                                ...
                                <button onClick={handleApplyCustomFilter} className="bg-orange-500 text-white p-2 rounded-xl shadow-lg transition-all active:scale-90"><FilterIcon size={14} /></button>
                            </div>
                        )}
                    </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <Card title="Infraestrutura de Dados" icon={<BarChart3 size={18} className="text-orange-500" />}>
                        <div className="py-10 text-center text-slate-400 flex flex-col items-center">
                            ...
                            <p className="text-[10px] font-medium text-slate-300 mt-2">Dados processados via Joins Relacionais do Banco.</p>
                        </div>
                    </Card>
                </div>
