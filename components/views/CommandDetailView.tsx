import React, { useState, useEffect, useMemo, useRef } from 'react';
    ...
    const CommandDetailView: React.FC<{ commandId: string; onBack: () => void }> = ({ commandId, onBack }) => {
        ...
        const handleFinishCheckout = async () => {
            // SEGURANÇA: Bloqueio se commandId ou client_id estiverem ausentes
            if (!commandId) {
                setToast({ message: "Erro crítico: ID da comanda não localizado.", type: 'error' });
                return;
            }
            ...
            try {
                // 1. VERIFICAÇÃO PREVENTIVA DE IDEMPOTÊNCIA
                ...
                if (existingPay) {
                    console.log('✅ Pagamento já liquidado no banco. Ignorando RPC.');
                } else {
                    // 2. REGISTRO FINANCEIRO VIA RPC (Obrigatório por regra de trigger)
                    const mainPayment = addedPayments[0];
                    const totalAmount = addedPayments.reduce((acc, p) => acc + p.amount, 0);

                    const { data: rpcResult, error: rpcError } = await supabase.rpc('register_payment_transaction', {
                        p_studio_id: activeStudioId,
                        p_professional_id: command.professional_id || null,
                        p_client_id: command.client_id || null, // Garante que o cliente seja passado
                        p_command_id: commandId, // Garante que a comanda seja passada
                ...
                <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center z-20 shadow-sm flex-shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-none mt-1">ID: {commandId.substring(0,8).toUpperCase()}</p>
                        </div>
                    </div>
                            ...
                            {(addedPayments.length > 0 || historyPayments.length > 0) && (
                                <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden animate-in slide-in-from-bottom-4">
                                    <header className="px-8 py-5 border-b border-slate-50 bg-emerald-50/50">
                                        <h3 className="font-black text-emerald-800 text-xs uppercase tracking-widest flex items-center gap-2"><CheckCircle size={16} /> Fluxo de Pagamento</h3>
                                    </header>
                                    <div className="divide-y divide-slate-50">
