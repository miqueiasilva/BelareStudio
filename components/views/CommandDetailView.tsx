...
interface PaymentEntry {
    ...
    fee_rate: number;
    fee_amount: number; 
    net_value: number; // Campo crítico para bater com a constraint 23502 do DB
    created_at?: string;
    brand?: string;
    method_id?: string; 
}
...
const CommandDetailView: React.FC<CommandDetailViewProps> = ({ commandId, onBack }) => {
    ...
    // Estados do Terminal Progressivo (UX Profissional)
    ...
    // Filtra as bandeiras configuradas no banco baseado no tipo selecionado no terminal
    ...
    const handleConfirmPartialPayment = () => {
        ...
        // CÁLCULO DE TAXAS ANTES DO REGISTRO (Garante que net_value nunca seja nulo)
        ...
        // Reset Terminal para o estado inicial
    ...
    const handleFinishCheckout = async () => {
        ...
        const correctCommandId = commandId || command?.id;
        if (!correctCommandId) {
            setToast({ message: "ID de comanda não identificado.", type: 'error' });
            return;
        }

        if (!command || isLocked || (addedPayments.length === 0 && historyPayments.length === 0)) return;
        ...
        try {
            // VERIFICAÇÃO DE IDEMPOTÊNCIA: Checa se já existe pagamento para esta comanda
            const { data: existingPayment } = await supabase
                .from('command_payments')
                .select('id')
                .eq('command_id', correctCommandId)
                .maybeSingle();
            ...
            if (!existingPayment) {
                ...
                for (const p of addedPayments) {
                    // 1. REGISTRA NA RPC FINANCEIRA (Gera Transação de Caixa)
                    const { data: txId, error: txErr } = await supabase.rpc('register_payment_transaction', {
                        ...
                        p_command_id: correctCommandId,
                        p_client_id: command.client_id
                    });

                    if (txErr && txErr.code !== '23505') throw txErr;

                    // 2. REGISTRA NA TABELA DE AUDITORIA DE COMANDAS
                    if (txId || (txErr && txErr.code === '23505')) {
                        const { error: cpErr } = await supabase.from('command_payments').insert([{
                            command_id: correctCommandId,
                        ...
                        if (cpErr && cpErr.code !== '23505') throw cpErr;
                    }
                }
            }
        ...
        <div className="h-full flex flex-col bg-slate-50 font-sans text-left overflow-hidden">
            ...
            <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center z-20 shadow-sm flex-shrink-0">
                ...
                <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${isLocked ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-orange-50 text-orange-600 border border-orange-100'}`}>
                    {isLocked ? 'Documento Liquidado' : 'Em Aberto'}
                </div>
            </header>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
                <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">
                                        ...
                                        <div key={p.id} className="px-8 py-5 flex items-center justify-between bg-white group animate-in slide-in-from-right-4">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[9px] font-black text-emerald-600 uppercase">Líquido em Conta: R$ {p.net_value.toFixed(2)}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-4">
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Terminal de Cobrança Progressivo */}
                        {!isLocked && (
                            <div className="bg-white rounded-[48px] p-8 border border-slate-100 shadow-sm space-y-6">
                                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 flex items-center gap-2">
                                    <Tag size={14} className="text-orange-500" /> Terminal Financeiro
                                </h4>
                                ...
                                <div className="bg-slate-50 p-6 rounded-[32px] border-2 border-slate-100 space-y-6 min-h-[320px] flex flex-col">
                                        ...
                                        <div className="space-y-4 animate-in slide-in-from-right-4 flex-1">
                                            ...
                                            <div className="grid grid-cols-1 gap-2">
                                                ...
                                                {filteredConfigs.length === 0 && (
                                                    <div className="text-center py-10 text-slate-400"><AlertTriangle size={32} className="mx-auto mb-2 opacity-20"/><p className="text-[10px] font-bold uppercase">Nenhum contrato ativo para {selectedType}</p></div>
                                                )}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {paymentStep === 'installments' && (
                                        <div className="space-y-6 animate-in slide-in-from-right-4 flex-1">
                                            ...
                                            <p className="text-[8px] text-slate-400 text-center uppercase font-bold">* Juros calculados conforme contrato bancário</p>
                                        </div>
                                    )}

                                    {/* PASSO 4: CONFIRMAR VALOR */}
                                    {paymentStep === 'confirm' && (
                                ...
                                >
                                    {isFinishing ? (
                                        ...
                                        <><CheckCircle size={24} /> FECHAR COMANDA</>
                                    )}
                                </button>
                            </div>
                        )}
                        ...
                        {isLocked && (
                            <div className="bg-emerald-50 p-8 rounded-[48px] border-2 border-emerald-100 text-center space-y-4 animate-in zoom-in-95">
                                ...
                                <p className="text-xs text-emerald-600 font-medium leading-relaxed">Este documento foi arquivado e o faturamento bruto foi consolidado no fluxo de caixa da unidade.</p>
                            </div>
                        )}
                    </div>
                </div>
