...
const CommandDetailView: React.FC<CommandDetailViewProps> = ({ commandId, onBack }) => {
    ...
    const handleInitPayment = (method: PaymentMethod) => {
        ...
        // Sugere o valor restante para este método
    ...
    const handleFinishPayment = async () => {
        ...
        try {
            ...
            // 1. Registrar CADA pagamento no financeiro de forma independente
            for (const payment of addedPayments) {
                const { error: finError } = await supabase.from('financial_transactions').insert([{
                    description: `Recebimento Parcial (${payment.method.toUpperCase()}) - Comanda #${command.id.split('-')[0].toUpperCase()} - ${clientName}`,
            ...
            // 2. Atualizar Comanda para "Pago"
            const { error: cmdError } = await supabase
                .from('commands')
                .update({ 
                    status: 'paid',
                    closed_at: new Date().toISOString(),
                    total_amount: totals.total // Valor líquido final (com desconto)
                })
                .eq('id', command.id);

            if (cmdError) throw cmdError;

            setToast({ message: "Conta fechada com sucesso! 💰", type: 'success' });
            setTimeout(onBack, 2000);
        } catch (e) {
            ...
            <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center z-20 shadow-sm flex-shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Múltiplos Pagamentos Disponíveis</p>
                    </div>
                </div>
                            ...
                            <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden animate-in slide-in-from-bottom-4">
                                <header className="px-8 py-5 border-b border-slate-50 bg-emerald-50/50">
                                    <h3 className="font-black text-emerald-800 text-xs uppercase tracking-widest flex items-center gap-2">
                                        <CheckCircle size={16} /> Pagamentos Confirmados
                                    </h3>
                                </header>
                                <div className="divide-y divide-slate-50">
                                    {addedPayments.map(p => (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-slate-900 rounded-[48px] p-10 text-white shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="relative z-10 space-y-6">
                                ...
                                {/* BALANÇO DE PAGAMENTO MISTO */}
                                {totals.paid > 0 && (
                                    <div className="bg-white/5 p-4 rounded-3xl border border-white/10 space-y-3">
                                        <div className="flex justify-between text-xs font-bold">
                                            <span className="text-slate-400">Já Lançado:</span>
                                            <span className="text-emerald-400">R$ {totals.paid.toFixed(2)}</span>
                                        </div>
                                        <div className="flex justify-between items-center pt-2 border-t border-white/5">
                                            <span className="text-xs font-black uppercase tracking-widest text-orange-400">Falta Pagar:</span>
                        ...
                        <div className="bg-white rounded-[48px] p-8 border border-slate-100 shadow-sm space-y-6">
                            ...
                            {/* Input de Valor Dinâmico ao selecionar método */}
                            {activeMethod ? (
                        ...
                        <div className="p-6 bg-blue-50 rounded-[32px] border border-blue-100 flex gap-4">
                            ...
                            <p className="text-[10px] text-blue-800 font-bold uppercase leading-relaxed">
                                Você pode combinar Pix, Dinheiro e Cartão na mesma conta. O sistema lançará cada parte individualmente no seu financeiro.
                            </p>
                        </div>
                    </div>
