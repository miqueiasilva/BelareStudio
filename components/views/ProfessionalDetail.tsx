...
const ProfessionalDetail: React.FC<ProfessionalDetailProps> = ({ professional: initialProf, onBack, onSave }) => {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            const [svcsRes, roomsRes] = await Promise.all([
                supabase.from('services').select('*').eq('studio_id', activeStudioId).order('nome'),
                supabase.from('resources').select('id, name').eq('studio_id', activeStudioId).eq('active', true).order('name')
            ]);

            if (svcsRes.data) setAllServices(svcsRes.data as any);
                                ...
                                <Card title="Visibilidade" icon={<Settings2 size={18} className="text-orange-500" />}>
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 transition-all hover:border-orange-200">
                                            ...
                                            <ToggleSwitch on={!!prof.show_in_calendar} onClick={() => setProf({...prof, show_in_calendar: !prof.show_in_calendar})} />
                                        </div>
                                        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 transition-all hover:border-orange-200">
                                            <div>
                                            ...
                                            <ToggleSwitch on={!!prof.online_booking_enabled} onClick={() => setProf({...prof, online_booking_enabled: !prof.online_booking_enabled})} />
                                        </div>
                                    </div>
                                </Card>
                            </div>
                                ...
                                <Card title="Informações Pessoais">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <div className="relative group">
                                                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-orange-500 transition-colors"><Armchair size={16} /></div>
                                                <select name="resource_id" value={prof.resource_id} onChange={handleInputChange} className="w-full bg-white border border-slate-200 rounded-xl py-2.5 pl-11 pr-10 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-orange-50 focus:border-orange-400 transition-all shadow-sm appearance-none">
                                    ...
                                    <div className="mt-6 space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Biografia e Notas</label><textarea value={prof.bio} name="bio" onChange={handleInputChange} className="w-full h-24 border border-slate-200 rounded-2xl p-4 focus:ring-2 focus:ring-orange-50 outline-none resize-none font-medium text-slate-600 shadow-sm" placeholder="Experiência profissional..." /></div>
                                </Card>
                            </div>
                        </div>
                    )}
